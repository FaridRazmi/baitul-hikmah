...
function proceedToCheckout() {
  ...
  const dateReturn = document.getElementById("borrowReturn").value.trim();
  ...
  if (!/^\d{2}\/\d{2}\/\d{2}$/.test(dateReturn)) {
    alert("Please enter the date in DD/MM/YY format.");
    document.getElementById("borrowReturn").focus();
    return;
  }
  ...
  localStorage.setItem(
    "borrowerDetails",
    JSON.stringify({ name, matric, date, dateReturn }),
  );
